 ...
 void loop() {
   ...
   if (system.getFanState()) outputs.activateFan();
   else outputs.deactivateFan();
   
   if (system.getHeaterState()) outputs.activateHeater();
   else outputs.deactivateHeater();
   
   if (system.getPumpState()) outputs.activateWaterPump();
   else outputs.deactivateWaterPump();
   
   if (system.getLightState()) outputs.activateLights();
   else outputs.deactivateLights();
 ...
 void updateDisplay(float temp, int moisture, int light, DateTime time, SystemState& state) {
   ...
   // Show active devices
   if (state.getFanState()) lcd.print("F");
   if (state.getHeaterState()) lcd.print("H");
   if (state.getPumpState()) lcd.print("W");
   if (state.getLightState()) lcd.print("L");
 }